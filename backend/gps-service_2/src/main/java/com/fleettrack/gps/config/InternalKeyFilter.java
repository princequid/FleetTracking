 ...
 * (The /ws handshake is gateway-proxied, so it carries the key too.)
 */
@Component
...
public class InternalKeyFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        // /ws is the websocket route (its own STOMP-level Bearer auth); exempt the
        // handshake/SockJS transport so realtime isn't broken by the key check.
